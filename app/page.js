import Image from "next/image";
import { faTelegram, faXTwitter } from "@fortawesome/free-brands-svg-icons";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";

export default function Home() {
  return (
    <>
      <div
        id="socialbar"
        class="flex justify-center bg-[#639F49] py-1 px-4 items-center"
      >
        <div className="flex justify-between">
          <a
            className="m-2"
            href="https://t.me/hopetokenchatofficial"
          >
            <FontAwesomeIcon
              icon={faXTwitter}
              size="lg"
              className="text-white hover:text-[#BFE963]"
              style={{ width: "24px", height: "24px" }}
            />
          </a>
          <a className="m-2" href="https://t.me/hopetokenchatofficial">
            <FontAwesomeIcon
              icon={faTelegram}
              size="lg"
              className="text-white hover:text-[#BFE963]"
              style={{ width: "24px", height: "24px" }}
            />
          </a>
        </div>
      </div>

      <div
        id="mainbar"
        class="flex justify-between py-1 px-4 items-center bg-[#639F49]"
      >
        <div>
          <img className="w-32 h-32 rounded-full" src="/main.jpeg" />
        </div>
        <div className="flex justify-center p-4">
          {/* <a className="px-2 md:text-2xl hover:text-[#639F49]" href="">
            Home
          </a>
          <a className="px-2 md:text-2xl hover:text-[#639F49]" href="">
            About
          </a> */}
          <a
            className="px-2 md:text-2xl text-white hover:text-[#BFE963]"
            href="#team"
          >
            Team
          </a>
        </div>
      </div>

      <div id="intro" className="flex justify-between p-4 flex-wrap">
        <div className="md:w-1/2">
          <img className="md:w-96 md:h-96 md:ml-28" src="/main.jpeg" />
        </div>
        <div className="md:w-1/2 px-4 py-2">
          <h2 className="text-[#639F49] text-5xl font-bold underline mt-4 md:mt-0">
            Hope Token
          </h2>
          <p className="p-4 pt-8 text-xl">
            "Hope Token: The digital currency that fuels dreams. With every
            transaction, you're supporting causes that inspire hope and change
            lives. Join a community driven by optimism, where every Hope Token
            is a beacon of possibility. Together, we're building a brighter
            future, one transaction at a time." 🌟💫✨
          </p>
          <div className="flex justify-center my-3">
            <a
              href="#about"
              className="px-4 py-2 cursor-pointer bg-[#639F49] text-white hover:bg-[#BFE963] rounded-lg"
            >
              About Us
            </a>
          </div>
        </div>
      </div>

      <div id="boxes" class="md:mt-10 mt-6 flex justify-center py-8 flex-wrap">
        <div class="w-full md:w-1/4 my-6 md:my-0 bg-[#639F49] shadow-md rounded-lg md:mx-4">
          <div class="relative">
            <img
              src="/main.jpeg"
              alt="Image 2"
              class="absolute top-0 w-12 h-12 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-full"
            />
          </div>
          <div class="p-4">
            <h2 class="text-xl font-bold mb-2 mt-4 text-white">Token Supply</h2>
            <p class="font-semibold text-white">
              Hope Token has a supply of 1,000,000,000 HOPE
            </p>
          </div>
        </div>

        <div class="w-full md:w-1/4 bg-[#639F49] my-6 md:my-0 shadow-md rounded-lg md:mx-4">
          <div class="relative">
            <img
              src="/main.jpeg"
              alt="Image 2"
              class="absolute top-0 w-12 h-12 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-full"
            />
          </div>
          <div class="p-4">
            <h2 class="text-xl font-bold mb-2 mt-4 text-white">
              Listed on 
            </h2>
            <p class="font-semibold text-white">
              Hope Token is listed on Binance Smart Chain
            </p>
          </div>
        </div>

        <div class="w-full md:w-1/4 bg-[#639F49] my-6 md:my-0 shadow-md rounded-lg md:mx-4">
          <div class="relative">
            <img
              src="/main.jpeg"
              alt="Image 2"
              class="absolute top-0 w-12 h-12 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-full"
            />
          </div>
          <div class="p-4">
            <h2 class="text-xl font-bold mb-2 mt-4 text-white">Token Name</h2>
            <p class=" font-semibold text-white">
              Hope token name is Hope Token (HOPE)
            </p>
          </div>
        </div>
      </div>

      <div
        id="banner"
        className="py-10 px-10 md:pb-14 flex items-center justify-center"
      >
        <img
          src="/cover.jpeg"
          alt="Pulsating Image"
          className="animated-image"
        />
      </div>

      <div id="about" className="p-6 flex justify-center flex-wrap">
        <div className="bg-[#639F49] rounded-lg flex justify-center flex-col items-center shadow-md m-2 p-4 md:w-2/5">
          <p className="text-3xl font-bold text-white">Token Distribution: </p>
          <p className="p-1 text-white">
            1. <span className="font-semibold">Community Rewards </span>: 30% -
            To incentivize and reward early supporters and contributors to the
            Hope Token ecosystem. <br />
            2. <span className="font-semibold">Charity Fund</span>: 40% -
            Allocated for funding charitable projects and initiatives that bring
            hope to communities in need. <br />
            3.<span className="font-semibold"> Development & Maintenance</span>:
            20% - Reserved for ongoing development, security, and maintenance of
            the Hope Token platform. <br />
            4. <span className="font-semibold">Marketing & Partnerships</span>:
            5% - For promoting and expanding the Hope Token's reach through
            marketing campaigns and strategic partnerships. <br />
            5. <span className="font-semibold">Team & Advisors</span>: 5% -
            Allocated for the core development team and advisors to ensure the
            project's long-term success. <br />
          </p>
        </div>
        <div className="bg-[#639F49] rounded-lg flex justify-start flex-col items-center p-4 md:w-2/5 shadow-md m-2">
          <p className="text-3xl font-bold text-white">Token Utility: </p>
          {/* <p className="p-1 italic font-bold text-white">Bardoge is listed on Arbitrum Chain.</p> */}
          <p className="text-white">
          <span className="font-semibold">- Staking Rewards</span>: Holders can stake their HOPE tokens to earn
            additional tokens or voting power in charity project decisions.{" "}
            <br />
            <span className="font-semibold">- Charity Voting</span>: HOPE holders can participate in governance to
            decide which charitable projects receive funding. <br />
            <span className="font-semibold">- Burn Mechanism</span>: A small percentage of tokens used in transactions will be burned, reducing the total supply over time, increasing
            scarcity, and potentially driving up the token's value. <br />
            <span className="font-semibold">- NFT Integration</span>: HOPE tokens can be used to mint and purchase NFTs
            related to charitable causes, further supporting the ecosystem.{" "}
            <br />
          </p>
        </div>
      </div>

      <section class="py-12 px-4 text-center" id="team">
        <h2 class="text-3xl font-semibold mb-8">Meet Our Partners</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class=" rounded-lg shadow-md p-6 bg-[#639F49]">
            <a href="https://dx.app"> 
              <img
                src="dxsale.png"
                alt="Team Member 1"
                class="w-32 h-32 rounded-full mx-auto mb-4"
              />
            </a>
            <h3 class="text-lg font-semibold mb-2 text-white">DxSale</h3>
            {/* <p class="text-gray-600">Lead Designer</p> */}
          </div>

          <div class="bg-[#639F49] rounded-lg shadow-md p-6 ">
            <img
              src="https://th.bing.com/th/id/OIP.1wTTp6-XPR-g6PS8D8ZAqAHaDt?pid=ImgDet&rs=1"
              alt="Team Member 2"
              class="w-32 h-32 rounded-full mx-auto mb-4"
            />
            <h3 class="text-lg font-semibold mb-2 text-white">BNB Chain</h3>
            {/* <p class="text-gray-600">Senior Developer</p> */}
          </div>

          <div class="bg-[#639F49] rounded-lg shadow-md p-6">
            <img
              src="https://th.bing.com/th/id/OIP.mleb5_jAlRxR3Yj8Zksc2gHaHa?pid=ImgDet&rs=1"
              alt="Team Member 3"
              class="w-32 h-32 rounded-full mx-auto mb-4"
            />
            <h3 class="text-lg font-semibold mb-2 text-white">Chain Link</h3>
            {/* <p class="text-gray-600">Marketing Manager</p> */}
          </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 my-4">
          <div class="bg-[#639F49] rounded-lg shadow-md p-6">
            <img
              src="https://pbs.twimg.com/profile_images/1653532864309239810/ZjT_zBAS_400x400.png"
              alt="Team Member 1"
              class="w-32 h-32 rounded-full mx-auto mb-4"
            />
            <h3 class="text-lg font-semibold mb-2 text-white">ARB</h3>
            {/* <p class="text-gray-600">Lead Designer</p> */}
          </div>

          <div class="bg-[#639F49] rounded-lg shadow-md p-6">
            <img
              src="data:image/jpeg;base64,"
              alt="Team Member 2"
              class="w-32 h-32 rounded-full mx-auto mb-4"
            />
            <h3 class="text-lg font-semibold mb-2 text-white">Coinbase</h3>
            {/* <p class="text-gray-600">Senior Developer</p> */}
          </div>

          <div class="bg-[#639F49] rounded-lg shadow-md p-6">
            <img
              src="https://th.bing.com/th/id/R.95bf06d513ed7ae2ea7ee2457a8daf91?rik=IfsTHJ7lZxWswQ&pid=ImgRaw&r=0"
              alt="Team Member 3"
              class="w-32 h-32 rounded-full mx-auto mb-4"
            />
            <h3 class="text-lg font-semibold mb-2 text-white">1inch</h3>
            {/* <p class="text-gray-600">Marketing Manager</p> */}
          </div>
        </div>
      </section>

      <div
        id="footer"
        className="p-4 py-10 pb-4 bg-[#639F49] flex justify-center flex-col items-center"
      >
        <p className="text-3xl font-bold text-white">Social Accounts</p>
        <div className="flex justify-between mt-6">
          <a
            className="m-2"
            href="https://t.me/hopetokenchatofficial"
          >
            <FontAwesomeIcon
              icon={faXTwitter}
              size="lg"
              className="w-8 h-8 text-white hover:text-[#BFE963]"
            />
          </a>
          <a className="m-2" href="https://t.me/hopetokenchatofficial">
            <FontAwesomeIcon
              icon={faTelegram}
              size="lg"
              className="w-8 h-8 text-white hover:text-[#BFE963]"
            />
          </a>
        </div>
      </div>

      <div
        id="endbar"
        class="flex justify-center bg-[#639F49] py-1 px-4 items-center"
      >
        <p className="font-bold text-white">Listed on Binance Smart Chain</p>
      </div>
    </>
  );
}
